...
class L1(BaseModule):
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        return np.sign(self.weights)  # fixing sub-gradient to be zero when not differentiable
...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        return np.asarray(- (1 / m) * np.sum(y * inner - np.log(1 + np.exp(inner))))
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        inner = self.weights @ X.T
        sigmoid = np.exp(inner) / (1 + np.exp(inner))
        return np.array((-1 / X.shape[0]) * (y @ X - X.T @ sigmoid))
...
class RegularizedModule(BaseModule):
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        reg_model = self.regularization_module_.compute_jacobian(**kwargs)
        if self.include_intercept_:
            reg_model = np.insert(reg_model, 0, 0)
        return self.fidelity_module_.compute_jacobian(**kwargs) + self.lam_ * reg_model
